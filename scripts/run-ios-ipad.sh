...
ipad_devices = [dev for dev in devices if "iPad" in dev.get("name", "")]

def pick_device():
    for name in preferred_names:
        for dev in ipad_devices:
            if dev.get("name") == name:
                return dev
    return ipad_devices[0] if ipad_devices else None
...
ipad_types = [dev for dev in device_types if "iPad" in dev.get("name", "")]

def pick_type():
    for name in preferred_names:
        for dev in ipad_types:
            if dev.get("name") == name:
                return dev
    return ipad_types[0] if ipad_types else None

device_type = pick_type()
if not device_type:
    sys.exit("No iPad device types found.")
...
if [[ -z "${udid}" ]]; then
  echo "No simulator UDID found. Pass one explicitly: scripts/run-ios-ipad.sh <UDID>" >&2
  exit 1
fi
